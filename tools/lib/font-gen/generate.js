...
(async function () {
    ...
    const addData = (header, data, footer) => {
        doth.push("");
        doth.push(`  /\/ ${header}`);
    ...
    const generateFont = (font, outline) => {
        ...
        for (let i = 33; i < 127; i++) {
            ...
            if (bitmap.padLeft < minPadLeft) {
                minPadLeft = bitmap.padLeft;
            }
            if (bitmap.padLeft > maxPadLeft) {
                maxPadLeft = bitmap.padLeft;
            }
            if (bitmap.padTop < minPadTop) {
                minPadTop = bitmap.padTop;
            }
            if (bitmap.padTop > maxPadTop) {
                maxPadTop = bitmap.padTop;
            }
            // Compute bitmap data
            let v = '';
